"""GrantGuard package entry point.

Provides the ``main`` function used by the ``grantguard`` console script and by
the top-level ``grantguard.py`` shim for ``uv run grantguard.py``.
"""
import argparse
import sys
...
def create_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="grantguard",
        description="GrantGuard — audit & clean your Claude Code permission allowlist",
    )
